...
7. Possible Issues: the list of genes used in the analysis was composed of all genes in the ClinVar database and all of the genes related to human-specific diseases in GWAS. This causes a problem where we compare the genes in each disease to genes related to other human-specific diseases. To correct this we can generate a list of genes related to a non-hunan-specific-disease (for example: diabetes) by utilising the GWAS catalog and using them as a comparison. More accurate would be to create such "null" gene list for each celltype separately to account for celltype-specific effects. We could use the genes from ClinVar that are not related to any of the disease-specific genes (~5000 genes), but this causes a problem because these are mostly monogenic diseases and the selection againt changes in these genes is expected to be stronger than in the null polygenic diseases.
